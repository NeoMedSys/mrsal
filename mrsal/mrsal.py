...
class Mrsal:
    ...
    def setup_queue_binding(self, exchange: str, queue: str, routing_key: str = None, arguments=None):
        ...
        try:
            ...
            if self.verbose:
                self.log.info(f"In such setup a message published to the exchange:{exchange} with routing key: {routing_key} will be routed to queue: {queue}")
            return bind_result
    ...
    def start_consumer(
        ...
        if fast_setup:
            ...
            try:
                ...
                self.log.error(f"I tried checking if the exchange and queue exist but failed with: {err}")
