...
def test_delay_letter():
    ...
    q_result: pika.frame.Method = amqp.setup_queue(queue=QUEUE)
    assert q_result != None

    # Bind main queue to the main exchange with routing_key
    qb_result: pika.frame.Method = amqp.setup_queue_binding(exchange=EXCHANGE,
    ...
    result = amqp.setup_queue(queue=QUEUE)
